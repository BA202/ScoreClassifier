...
class BERT_TextClassification_Production:
    ...
    def __init__(self,trainingData= None,**kwargs):
        ...
        trainingData = [[sample[0], self.__catToInt[sample[1]]] for sample in trainingData]
        ...
        self.__tokenizer = BertTokenizerFast.from_pretrained(pretrained_model_name_or_path=model_name, config=config)
        transformer_model = TFBertModel.from_pretrained(model_name,config=config)

        bert = transformer_model.layers[0]

        input_ids = Input(shape=(self.__max_length,), name='input_ids', dtype='int32')
